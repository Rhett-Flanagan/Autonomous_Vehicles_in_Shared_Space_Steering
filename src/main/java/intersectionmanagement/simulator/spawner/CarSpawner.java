...
public class CarSpawner extends Actor {
    ...
    public CarSpawner(Simulator simulator, Node spawnLocation, byte[] serializedNetwork, int simulationSteps, Function function, double[] params, double randomness, int config) {
        super(simulator, spawnLocation);
        speed = 0f;
        radius = 0f;
    ...
    public void step(ArrayList<Actor> actorArray) {
        ...
        if (counter <= 0) {
            ...
            simulator.addCar(new Car(simulator, target, new NeuralNetworkController(serializedNetwork), config));
        }
